...
create table if not exists upload_jobs (
  ...
  commit_sha text,
  error_message text,
  -- Real server-side commit progress (see bulkCommitService.commitFiles),
  -- polled by the client during /commit so the progress bar reflects
  -- actual blob/tree/commit work instead of a simulated curve.
  progress_stage text,
...
-- ============================================================
-- user_vercel_connections
-- Independent of github_connections — a user can connect GitHub only,
-- Vercel only, or both. See sql-supabase/003_vercel_connections.sql for
-- the full migration notes (same table, same semantics).
-- ============================================================
